...
public class CandidateController : BaseController
{
    ...
    [AllowAnonymous]
    [HttpGet("about-me")]
    public async IAsyncEnumerable<string> GetAboutMe([FromQuery] string prompt)
    {
        await foreach (var message in _userService.GetStreamingResponse(prompt))
        {
            yield return message;
            await Task.Delay(100); // Adjust as necessary for your streaming speed
        }
    }
    ...
    {
        string _apiKey = "AIzaSyD1rX7VkFWhjoEtYUl48DgMFjM2wFl4C7M";
 ...
 requestBody = JsonConvert.SerializeObject(
                new
                {
                    contents = new List<Dictionary<string, List<Dictionary<string, string>>>>
                    {
            ...
            var response = await client.PostAsync($"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key={_apiKey}", content);
 ...
 object>>(responseString);
            ...
            {
                throw new Exception($"Error generating text: {await response.Content.ReadAsStringAsync()}");
            }
        }
    }
}
