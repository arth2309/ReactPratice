...
const Cryptotable = (props: any) => {
  ...
  const searchHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filteredData = list.filter((item: coindetails) =>
  ...
  const handleCoinDetail = (id: string) => {
    navigate(`/Coindetails/${id}`, { state: { list: list } });
  };

  return (
    <div className="px-5 py-3 mt-2 bg-body-tertiary">
      <div className="d-flex flex-column align-items-center mb-2">
        <h1 className="tabletitle">Cryptocurrency Prices by Market Cap</h1>
      </div>
      ...
      <Table striped responsive className="mt-4">
        <thead className="table-success">
          <tr>
            ...
            >
              Coin {sortField === "name" && (sortOrder === "asc" ? "▲" : "▼")}
            </th>
            <th
              style={{ cursor: "pointer" }}
            ...
            >
              Market Cap{" "}
              {sortField === "market_cap" && (sortOrder === "asc" ? "▲" : "▼")}
            </th>
          </tr>
        </thead>
        <tbody>
            ...
            <tr key={item.id}>
              <td>
                <div
                  ...
                  style={{ cursor: "pointer" }}
                  onClick={() => handleCoinDetail(item.id)}
                >
                  <img
                    src={item.image}
                    alt={item.name}
