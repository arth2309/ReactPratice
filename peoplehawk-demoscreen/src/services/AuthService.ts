...
export const Register = async (
  ...
  try {
    ...
    showToast(
      TOAST.EMAIL_ALREADY_EXIST.title,
      error.response.data.error,
      TOAST.EMAIL_ALREADY_EXIST.type
    );
    return null;
  }
};
...
export const Login = async (data: LoginFormValues): Promise<string | null> => {
  try {
    ...
    showToast(
      TOAST.INVALID_CREDENTIALS.title,
      TOAST.INVALID_CREDENTIALS.description,
      TOAST.INVALID_CREDENTIALS.type
    );
    return null;
  }
};
